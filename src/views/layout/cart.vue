  ...
  <div class="cart">
    ...
    <div v-if="isLogin && cartList.length > 0">
      ...
      <div class="footer-fixed">
        <div @click="allChendFn" class="all-check">
        ...
        <div class="all-total">
          ...
          <div v-else @click="handleDel" class="delete">删除</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...mapActions('cart', ['getCartListAction', 'delSelAction']),

    checkFn (goodsId) {
      this.$store.commit('cart/checkFn', goodsId)
    },
    allChendFn () {
      this.$store.commit('cart/AllCheckFn', !this.isAllChecked)
    },
  ...
  created () {
    console.log(this.isLogin, this.cartList.length)
    if (this.isLogin) {
  ...
  watch: {
    isEdit (newValue) {
      if (newValue) {
        this.$store.commit('cart/AllCheckFn', false)
      } else {
        this.$store.commit('cart/AllCheckFn', true)
      }
    }
  }
}
